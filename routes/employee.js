...
const BASE_URL = process.env.BASE_URL || 'https://trackzone-backend-os95.onrender.com';
const OFFICE_LAT = 10.8302128;
const OFFICE_LON = 77.0679478;
const GEOFENCE_RADIUS = 5000;

const { upload } = require('../utils/Cloudinary'); // Assuming Cloudinary utility for file uploads
...
router.get('/leave/all', async (req, res) => {
  try {
    ...
    // Manually populate employee details since we can't use ref
    const leavesWithEmployee = await Promise.all(leaves.map(async (leave) => {
      const employee = await Employee.findOne({ employeeId: leave.employee });
      return {
        ...leave._doc,
        employee: employee ? { name: employee.name, email: employee.email, employeeId: employee.employeeId } : null,
      };
    }));

    res.status(200).json({ message: 'All leave requests fetched successfully.', data: leavesWithEmployee });
  } catch (err) {
...
router.put('/leave/:leaveId', async (req, res) => {
  ...
  try {
    ...
    const notification = new Notification({
      ...
      message: `Your leave request for ${leave.leaveType} from ${new Date(leave.startDate).toLocaleDateString()} to ${new Date(leave.endDate).toLocaleDateString()} has been ${status}. ${adminComment ? `Comment: ${adminComment}` : ''}`,
      recipients: { type: 'specific', value: employee.email },
      priority: 'High',
    });
    await notification.save();
    ...
    const html = `
      ...
      <p>Your leave request for <strong>${leave.leaveType}</strong> from <strong>${new Date(leave.startDate).toLocaleDateString()}</strong> to <strong>${new Date(leave.endDate).toLocaleDateString()}</strong> has been <strong>${status}</strong>.</p>
      ${adminComment ? `<p><strong>Admin Comment:</strong> ${adminComment}</p>` : ''}
      <p>Regards,<br/>Attendance System</p>
    `;
    await sendMail(employee.email, `Leave Request ${status.charAt(0).toUpperCase() + status.slice(1)}`, html);

    const io = req.app.get('io');
    io.emit('leave_update_notification', {
      employee: employee.name,
      leaveType: leave.leaveType,
      status,
      message: `Leave request for ${employee.name} has been ${status}.`,
    });

    res.status(200).json({ message: `Leave request ${status} successfully.`, data: leave });
  } catch (err) {
...
router.get('/dashboard/:employeeId/work-metrics', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Query check-ins using employeeId
    ...
    // Query leave count using employeeId
    ...
    console.error('Work Metrics Error:', err);
    res.status(500).json({ message: 'Error fetching work metrics' });
  }
});
...
router.put('/attendance/:employeeId/:year/:month', verifyToken, async (req, res) => {
  try {
    const { employeeId, year, month } = req.params;
    const { day, status, checkInTime, checkOutTime } = req.body;
    ...
    if (updatedMonthData) {
      updatedMonthData.days = updatedMonthData.days.map(day => ({
        ...
        hoursWorked: day.checkOutTime && day.checkInTime 
          ? (day.checkOutTime - day.checkInTime) / 3600000
          : day.hoursWorked || 0
      }));
